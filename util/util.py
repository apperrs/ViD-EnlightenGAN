...
def atten2im_dynamic(image_tensor, imtype=np.uint8, gamma=1.5):
    image_tensor = image_tensor[0]
    image_tensor = torch.cat((image_tensor, image_tensor, image_tensor), 0)
    image_numpy = image_tensor.cpu().float().numpy()
    image_numpy = (np.transpose(image_numpy, (1, 2, 0))) * 255.0
    ...
    image_numpy = np.power(image_numpy, 1/gamma)
    image_numpy = image_numpy + 1 - image_numpy.max()

    return (image_numpy * 255.0).astype(imtype)
