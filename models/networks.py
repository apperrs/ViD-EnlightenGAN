...
class TODIAM(nn.Module):
    def __init__(self, opt):
        super(TODIAM, self).__init__()
        self.opt = opt

        self.conv_gray = nn.Conv2d(3, 3, 3, 1, padding=1)
        self.gray_LReLU = nn.LeakyReLU(0.2, inplace=True)
        self.odconv_gray = ODConv2d(3, 1, 7, 1, padding=3)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, input):
        gray_1 = self.conv_gray(input[:, :3, :, :])
        gray_1 = self.gray_LReLU(gray_1)
        gray_1 = self.odconv_gray(gray_1)
        gray_1 = self.sigmoid(gray_1)

        gray_2 = self.conv_gray(input[:, 3:, :, :])
        gray_2 = self.gray_LReLU(gray_2)
        gray_2 = self.odconv_gray(gray_2)
        gray_2 = self.sigmoid(gray_2)

        gray = 1. - torch.cat((gray_1, gray_2), 1)

        if self.opt.self_attention:
            gray_2_down = self.downsample_1(gray)
            gray_3_down = self.downsample_2(gray_2_down)
            gray_4_down = self.downsample_3(gray_3_down)
            gray_5_down = self.downsample_4(gray_4_down)
            return gray, gray_2_down, gray_3_down, gray_4_down, gray_5_down
        else:
            return gray
...
class FSAM(nn.Module):
    ...
    def forward(self, input, gray):
        concat_features = torch.cat((input, gray), 1)
        b, c, h, w = concat_features.size()

        avg_out = self.avg_pool(concat_features).view(b, c)
        avg_out = self.fc(avg_out).view(b, c, 1, 1)

        max_out = self.max_pool(concat_features).view(b, c)
        max_out = self.fc(max_out).view(b, c, 1, 1)

        channel_attention = avg_out + max_out
        channel_refined = concat_features * channel_attention.expand_as(concat_features)

        avg_out_spatial = torch.mean(channel_refined, dim=1, keepdim=True)
        max_out_spatial, _ = torch.max(channel_refined, dim=1, keepdim=True)
        spatial_attention_input = torch.cat([avg_out_spatial, max_out_spatial], dim=1)
...
class Unet_resize_conv(nn.Module):
    def __init__(self, opt, skip):
        ...
        self.todiam = TODIAM(opt)

        self.fsam = FSAM(8)

        if opt.self_attention:
            self.conv1_1 = nn.Conv2d(8, 32, 3, padding=p)
        else:
    ...
    def forward(self, input):
        ...
        if self.opt.self_attention:
            gray, gray_2, gray_3, gray_4, gray_5 = self.todiam(input)
        else:
            gray = self.todiam(input)

        fsam_output = self.fsam(input, gray)
        x = self.bn1_1(self.LReLU1_1(self.conv1_1(fsam_output)))
        ...
        x = x * gray_5[:, 1, :, :].unsqueeze(1) if self.opt.self_attention else x
        conv5 = self.bn5_2(self.LReLU5_2(self.conv5_2(x)))

        conv5 = F.upsample(conv5, scale_factor=2, mode='bilinear')
        conv4 = conv4 * gray_4[:, 1, :, :].unsqueeze(1) if self.opt.self_attention else conv4
        up6 = torch.cat([self.deconv5(conv5), conv4], 1)
        x = self.bn6_1(self.LReLU6_1(self.conv6_1(up6)))
        conv6 = self.bn6_2(self.LReLU6_2(self.conv6_2(x)))

        conv6 = F.upsample(conv6, scale_factor=2, mode='bilinear')
        conv3 = conv3 * gray_3[:, 1, :, :].unsqueeze(1) if self.opt.self_attention else conv3
        up7 = torch.cat([self.deconv6(conv6), conv3], 1)
        x = self.bn7_1(self.LReLU7_1(self.conv7_1(up7)))
        conv7 = self.bn7_2(self.LReLU7_2(self.conv7_2(x)))

        conv7 = F.upsample(conv7, scale_factor=2, mode='bilinear')
        conv2 = conv2 * gray_2[:, 1, :, :].unsqueeze(1) if self.opt.self_attention else conv2
        up8 = torch.cat([self.deconv7(conv7), conv2], 1)
        x = self.bn8_1(self.LReLU8_1(self.conv8_1(up8)))
        conv8 = self.bn8_2(self.LReLU8_2(self.conv8_2(x)))

        conv8 = F.upsample(conv8, scale_factor=2, mode='bilinear')
        conv1 = conv1 * gray[:, 1, :, :].unsqueeze(1) if self.opt.self_attention else conv1
        ...
        latent = latent * gray[:, 1, :, :].unsqueeze(1)
...
def define_D_N():
    ...
    model = D_N(num_classes=1).to(device)
